...
// export const diagnosticRe = /^\<stdin\>:(\d+):(\d+):(?:((?:\{.+?\})+):)?
// ((?:fatal )?error|warning): (.*?)$/;
export const diagnosticRe =
    /^[^:]+:(\d+):(\d+):(?:((?:\{.+?\})+):)? ((?:fatal )?error|warning): (.*?)$/;
...
export interface DiagnosticProvider {
  provideDiagnostic(
      ...
      token: vscode.CancellationToken): Thenable<vscode.Diagnostic[]>;
}
...
export function registerDiagnosticProvider(
    selector: vscode.DocumentSelector, provider: DiagnosticProvider,
    name: string): vscode.Disposable {
  ...
  vscode.workspace.onDidChangeTextDocument((change) => {
    ...
    delay(cancellers.get(uriStr).token)
        .then(() => {
          cancellers.get(uriStr).dispose();
          cancellers.set(uriStr, new vscode.CancellationTokenSource);
          return provider.provideDiagnostic(
              change.document, cancellers.get(uriStr).token);
        })
        .then(
            (diagnostics) => {
              cancellers.get(uriStr).dispose();
...
export class ClangDiagnosticProvider implements DiagnosticProvider {
            ...
            (e: execution.FailedExecution) => {
              if (e.errorCode === execution.ErrorCode.BufferLimitExceed) {
                vscode.window.showWarningMessage(
                    'Diagnostic was interpreted due to rack of buffer size. The buffer size can be in' +
                    'creased using `clang.diagnostic.maxBuffer`. ');
              }
              return '';
            });
  }
  ...
  fetchDiagnostic(
      document: vscode.TextDocument,
      token: vscode.CancellationToken): Thenable<string> {
    ...
    data.split(/\r\n|\r|\n/).forEach((line) => {
      ...
      if (matched[3] == null) {
        ...
        range = new vscode.Range(
            ranges[0].start.line - 1, ranges[0].start.character - 1,
